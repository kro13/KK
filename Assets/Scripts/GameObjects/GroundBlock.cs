...
namespace Assets.Scripts.GameObjects
{
    public class GroundBlock : MonoBehaviour, ICollideable
    {
        ...
        public static float borderWidth = 0.1f;
        ...
        private void SetDepth(int depth)
        {
            ...
            foreach (Transform child in transform)
            {
                ...
                {
                    case "Snow":
                        color = new Color(1f - 0.2f * depth, 1f - 0.2f * depth, 1f - 0.1f * depth);
                        break;
                    case "Spot":
                        color = new Color(1f - 0.2f * (depth + 1), 1f - 0.2f * (depth + 1), 1f - 0.1f * (depth + 1));
                        break;
                    case "Border":
                        color = new Color(1f - 0.5f - 0.05f * (depth + 1), 1f - 0.5f - 0.05f * (depth + 1), 1f - 0.4f - 0.05f * (depth + 1));
                        break;
                    case "Pine":
